  ...
  module Helpers
    ...
    module ViewHelpers
      ...
      def render_action_link(link, url_options)
        ...
        html_options[:id] = action_link_id("#{id_from_controller(url_options[:controller]) + '_' if url_options[:controller]}" + url_options[:action],url_options[:id] || url_options[:parent_id])
