  ...
  module Helpers
    ...
    module ViewHelpers
      ...
      def render_action_link(link, url_options, record = nil, html_options = {})
        ...
        html_options[:id] = action_link_id("#{id_from_controller(url_options[:controller]) + '-' if url_options[:parent_controller]}" + "#{url_options[:association].to_s + '-' if url_options[:association]}" + url_options[:action].to_s,url_options[:id] || url_options[:parent_id])
